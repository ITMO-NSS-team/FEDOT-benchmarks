...
def validation_metric_classification(chain: Chain, dataset_to_validate: InputData) -> float:
...
def validation_metric_regression(chain: Chain, dataset_to_validate: InputData) -> float:
    # the execution of the obtained composite models
    predicted = chain.predict(dataset_to_validate)
    # the quality assessment for the simulation results
...
def run_credit_scoring_problem(train_file_path, test_file_path,
                               ...
                               start_depth: int = 3, task: Task = Task(TaskTypesEnum.classification)):
    ...
    calculate_validation_metric = validation_metric_classification
    if task.task_type == TaskTypesEnum.regression:
        calculate_validation_metric = validation_metric_regression
    chains_roc_auc = []
    chains_complexity = []
    if type(metrics) is list:
        if len(metrics) > 1:
